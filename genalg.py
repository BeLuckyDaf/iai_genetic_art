        ...
        if mutated:
            ...
            for cy in range(y, min(y+block_size, image_length_y)):
                for cx in range(x, min(x+block_size, image_length_x)):
                    #g = random.randint(max(image[cy, cx, 1] - offset, 0), min(image[cy, cx, 1] + offset, 255))
                    #r = random.randint(max(image[cy, cx, 2] - offset, 0), min(image[cy, cx, 2] + offset, 255))
                    #b = random.randint(max(image[cy, cx, 0] - offset, 0), min(image[cy, cx, 0] + offset, 255))
                    image[cy, cx, 0] = min(max(image[cy, cx, 0]+b, 0), 255)
                    image[cy, cx, 1] = min(max(image[cy, cx, 1]+g, 0), 255)
                    image[cy, cx, 2] = min(max(image[cy, cx, 2]+r, 0), 255)
...
@jit(nopython=True)
def fitness(image, target, pass_points):
    image_length_x = len(image[0])
    image_length_y = len(image)
    passed = 0
    ...
    for y in range(0, image_length_y):
        for x in range(0, image_length_x):
            ...
            points /= 3
            if points > 0:
                passed += min(pass_points/points, 1)
    return passed/total
...
def get_fitlist(population, target, pass_points):
    result = []
    for p in population:
        fit = fitness(p, target, pass_points)
        result.append(fit)
    return result


def calculate(image, target, epochs=50, pass_points=10, mutation_rate=0.5, offset=50, select=5, initial_pop=30):
    population = [image]
    ...
    fitlist = get_fitlist(population, target, pass_points)
    ...
    for iteration in range(epochs):
        ...
        fitlist = get_fitlist(population, target, pass_points)
        new_population = selection(population, fitlist, select)
    fitlist = get_fitlist(new_population, target, pass_points)
    return new_population[0]


def calculate_image(image, target, epochs=10, pass_points=10, mutation_rate=0.5, offset=50, select=5, initial_pop=30):
    ...
    for y in range(0, image_length_y, 2):
        for x in range(0, image_length_x, 2):
            point = calculate(image_copy[y:y+2, x:x+2], target[y:y+2, x:x+2], epochs, pass_points, mutation_rate, offset, select, initial_pop)
